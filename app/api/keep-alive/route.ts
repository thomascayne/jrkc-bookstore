...
import { determineAction, generateRandomString, QueryResponse } from './helper'
...
const querySupabase = async (supabase: SupabaseClient, randomStringLength: number = 12): Promise<QueryResponse> => {
    ...
    const { data, error } = await supabase.rpc('keep_alive_operations', {
        operation: 'retrieve',
        column_name: config.column,
        value: currentRandomString
    })
